...
export function buildCard(info: ArticleInfo): HTMLElement {
    ...
    `<a href="/${info.category}/${info.slug}" class="article-card-link">` +
      `<div class="article-card-topline">` +
        ...
        `<span class="card-engagement" style="display:none;">` +
          `<span class="card-likes" style="display:none;">${likeSvg}<span class="card-likes-count"></span></span>` +
          `<span class="card-comments" style="display:none;">${commentSvg}<span class="card-comments-count"></span></span>` +
        `</span>` +
      `</div>` +
